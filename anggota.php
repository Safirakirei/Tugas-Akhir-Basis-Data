...
if(isset($_POST['esimpan'])){
    ...
    if($_GET['hal']=="edit"){
        ...
        $simpan = mysqli_query($koneksi,"INSERT INTO anggota (id_anggota, nama, no_telp)
...
if(isset($_GET['hal'])){
    ...
    if($_GET['hal']=="edit"){
        ...
        $tampil = mysqli_query($koneksi,"SELECT * FROM anggota WHERE id_anggota='$_GET[id]'");
        $data = mysqli_fetch_array($tampil);
        if($data2){
            //jika data ditemukan maka data ditampung ke variabel
            $vidanggota = $data['id_anggota'];
            $vnama =  $data['nama'];
            $vnotelp = $data['no_telp'];
        }
    }
    else if($_GET['hal']=="hapus"){
        //persiapan hapus data
        $hapus = mysqli_query($koneksi,"DELETE FROM anggota WHERE id_anggota='$_GET[id]' ");
        if($hapus){
            echo "<script>
...
<html>
    ...
    <body>
        <div class="container">
        <h1 class="text-center mt-4">FORM ANGGOTA</h1>
    <!-- awal card form -->
    <div class="card mt-3">
    ...
    <form action="" method="post">
        ...
        <div class="form-group">
            <label>No. Telp</label>
            <input type="text" name="tnotelp" value="<?=@$vnotelp?>" class="form-control" placeholder="input nomor telpon anda disini!" required>
        </div>

        <button type="submit" class="btn btn-success mt-3" name="esimpan">SIMPAN</button>
        <button type="reset" class="btn btn-danger mt-3" name="ereset">RESET</button>
    </form>
    </div>
    </div>
    <!-- akhir card form-->
        ...
        <table class="table table-bordered table-striped">
            ...
            while($data = mysqli_fetch_array($tampil)):
                ...
                <tr>
                    ...
                    <td><?=$data['no_telp']?></td>
                    <td>
                        <a href="anggota.php?hal=edit&id=<?=$data['id_anggota']?>" class="btn btn-warning">EDIT</a>
                        <a href="anggota.php?hal=hapus&id=<?=$data['id_anggota']?>" onclick="return confirm('apakah yakin ingin menghapus data ini?')" class="btn btn-danger">HAPUS</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </table>
